...
with app.test_client() as client:
    ...
    def test_delete_todo():
        res = client.post("/", json={"title": "hello world 3"})
        res_json = res.get_json()
        delete_res = client.delete(f"/{res_json['id']}")
        delete_res_json = delete_res.get_json()
        assert delete_res_json["title"] == "hello world 3"
        get_res = client.get(f"/{res_json['id']}")
        assert get_res.status_code == 404
        del_res = client.delete(f"/{res_json['id']}")
        assert del_res.status_code == 404

    @pytest.mark.it("it should update a single todo")
    def test_update_todo():
        res = client.post("/", json={"title": "hello world 4"})
        res_json = res.get_json()
        patch_res = client.patch(
            f"/{res_json['id']}", json={"completed": True})
        assert patch_res.status_code == 200
        get_res = client.get(f"/{res_json['id']}")
        get_res_json = get_res.get_json()
        assert get_res_json["completed"] == True
        bad_res = client.patch(
            f"/{res_json['id']}", json={"title": ""})
        assert bad_res.status_code == 400
        missing_res = client.patch(f"/{uuid.uuid4()}")
